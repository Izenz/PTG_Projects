// Ejercicios 1, 2 y 3 (parte 1) del tema 2
...
GLuint vboHandle;
  ...
  //Estas están obsoletas en el ejecicio2
  ...
  GLuint loc = glGetAttribLocation(programShaderID, "aPosition");
  glEnableVertexAttribArray(loc);
  glVertexAttribPointer(loc, 4, GL_FLOAT, GL_FALSE, 0, 0);

  GLuint loc2 = glGetAttribLocation(programShaderID, "aColor");
  glEnableVertexAttribArray(loc2);
  glVertexAttribPointer(loc2, 4, GL_FLOAT, GL_FALSE, 0, (char*)NULL + sizeof(GL_FLOAT) * 4 * 36);
...
bool init()
{
	...
	// Tarea por hacer (ejer. 1): Aquí creamos el objeto programa.
	vertexShaderID = glCreateShader(GL_VERTEX_SHADER);
	loadSource(vertexShaderID, "tema2_ej1.vert");
	fragmenShaderID = glCreateShader(GL_FRAGMENT_SHADER);
	loadSource(fragmenShaderID, "tema2_ej1.frag");
...
void display()
{
	...
	// Tarea por hacer (ejer. 3): Sustituir el anterior código por el código siguiente.
	...
	// Tarea por hacer (ejer. 1): Aquí activamos el objeto programa.
	glUseProgram(programShaderID);
	glUniformMatrix4fv(loc3, 1, GL_FALSE, &mvp[0][0]);
	draw1Cube();

	// Tarea por hacer (ejer. 1): Aquí lo desactivamos.
	glUseProgram(0);
	glutSwapBuffers();
}
...
void keyboard(unsigned char key, int x, int y)
{
	...
	case 27 : case 'q': case 'Q':
		// Tarea por hacer (ejer. 1): Aquí borramos el objeto programa.
