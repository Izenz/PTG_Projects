...
GLboolean textPasteMode = true;
GLboolean textRepeatMode = false;
GLint activeLight = 0;
GLint activeFilter = 0;
...
void keyboard(unsigned char key, int x, int y)
{
	...
	case 'm': case 'M':
		...
		textPasteMode = !textPasteMode;
		glTexEnvi(GL_TEXTURE_ENV, GL_TEXTURE_ENV_MODE, textPasteMode ? GL_REPLACE : GL_MODULATE);
		break;
	case 'e': case 'E':
		// Tarea por hacer: modo de repetición de la textura
		textRepeatMode = !textRepeatMode;

		glTexParameterf(GL_TEXTURE_2D, GL_TEXTURE_WRAP_S, textRepeatMode ? GL_CLAMP : GL_REPEAT);
		glTexParameterf(GL_TEXTURE_2D, GL_TEXTURE_WRAP_T, textRepeatMode ? GL_CLAMP : GL_REPEAT);
