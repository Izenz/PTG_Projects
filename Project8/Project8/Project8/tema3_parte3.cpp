...
GLuint locUniformApplyNMap, locUniformApplyCMap, locUniformApplyGloss, locUniformApplyHMap;

GLuint APPLY_HEIGHT = 0;

int numVertTeapot, numVertSphere, numVertPlane, numVertTorus;
bool APPLY_NORMAL_MAP = false, APPLY_COLOR_MAP = false, APPLY_GLOSS = false;
...
bool init()
{
	...
	// Tarea por hacer: Localización de los sampler con los mapas
	locUniformNormalMap = glGetUniformLocation(programID, "uNormalMap");
	locUniformApplyNMap = glGetUniformLocation(programID, "uApplyNM");
	locUniformColorMap = glGetUniformLocation(programID, "uColorMap");
	locUniformApplyCMap = glGetUniformLocation(programID, "uApplyCM");
	locUniformGlossMap = glGetUniformLocation(programID, "uGlossMap");
	locUniformApplyGloss = glGetUniformLocation(programID, "uApplyGloss");
	locUniformHeightMap = glGetUniformLocation(programID, "uHeightMap");
	locUniformApplyHMap = glGetUniformLocation(programID, "uApplyHM");
	...
	// Tarea por hacer: crear los objetos textura y cargar las imágenes
	
	// Normal map
	//char textureFile[] = "normal_map.tga";
	char textureNormals[] = "brick_normal.tga";
	char textureFile[] = "brick_color.tga";
	char textureGloss[] = "brick_gloss.tga";
	char textureHeight[] = "brick_height.tga";
	if ( LoadTGAFile(textureNormals, &tgaImage) )
	{
	...
	// Color map
	if ( LoadTGAFile(textureFile, &tgaImage) )
	{
	...
	// Gloss map
	if ( LoadTGAFile(textureGloss, &tgaImage) )
	{
	...
	// Height map
	if ( LoadTGAFile(textureHeight, &tgaImage) )
	{
...
void display()
{
	...
	// Tarea por hacer: pasar otras variables uniform comunes a los dos objetos
	glUniform1i (locUniformNormalMap, 0);	// GL_TEXURE0
	glUniform1i(locUniformColorMap, 1);		//GL_TEXTURE1
	glUniform1i(locUniformGlossMap, 2);		//GL_TEXTURE2
	glUniform1i(locUniformHeightMap, 3);	//GL_TEXTURE3

	glUniform1i(locUniformApplyNMap, APPLY_NORMAL_MAP ? 1 : 0);
	glUniform1i(locUniformApplyCMap, APPLY_COLOR_MAP ? 1 : 0);
	glUniform1i(locUniformApplyGloss, APPLY_GLOSS ? 1 : 0);
	glUniform1i(locUniformApplyHMap, APPLY_HEIGHT);
